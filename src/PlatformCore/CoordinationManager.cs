...
namespace PlatformCore
{
	...
	public class CoordinationManager : IDisposable
	{
		...
		public void Start() {
			...
			if (replicas.Count < 1) {
				// Trace.WriteLine("CoordinationManager not replicating anything since there are not
				// enough workers to support replication.");
				return;
			}
		...
		public void ReplicaAliveSignal(Uri workerUrl) {
			lock (rmanagerMutex) {
				...
				// Trace.WriteLine("CoordinationManager received ping signal from replica/worker
				// URL: '" + workerUrl
		...
		private void StatusUpdate(object state) {
			...
			foreach (var replica in replicas) {
				try {
					// Trace.WriteLine("CoordinationManager sending JobTracker state to
					// ReplicaWorker ID:"
					// + replica.WorkerId + ".");
